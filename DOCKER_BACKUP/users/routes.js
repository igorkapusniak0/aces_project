...
import {registerUser,authenticateUser,getUserEvents,getGlobalLeaderboard,setUserLocation,getAllLocal,calculateWeeklyTotalPoints,weeklyReport,getUserPosition,getLocalLeaderboard} from './functions.js';
import asyncHandler from 'express-async-handler';
const router = express.Router();

// wget --method=POST --header="Content-Type: application/json" --body-data='{"name":"bob"}' http://192.168.178.181:3000/users/register
...
// wget --method=POST --header="Content-Type: application/json" --body-data='{"name":"bob"}' http://192.168.178.181:3000/users/login
router.post('/login', asyncHandler(async (req, res) => {
    try {
        console.log("Incoming request:", req.query, req.body);
        const user_id = await authenticateUser(req.body.name);
